...
import com.application.cloud.auth.support.CustomeOAuth2AccessTokenGenerator;
import com.application.cloud.auth.support.core.CloudDaoAuthenticationProvider;
import com.application.cloud.auth.support.core.CustomeOAuth2TokenCustomizer;
import com.application.cloud.auth.support.core.FormIdentityLoginConfigurer;
...
import com.application.cloud.auth.support.password.OAuth2ResourceOwnerPasswordAuthenticationConverter;
import com.application.cloud.auth.support.password.OAuth2ResourceOwnerPasswordAuthenticationProvider;
import com.application.cloud.auth.support.sms.OAuth2ResourceOwnerSmsAuthenticationConverter;
...
import com.application.cloud.common.security.handler.FormAuthenticationFailureHandler;
import com.application.cloud.common.security.handler.SsoLogoutSuccessHandler;
...
public class AuthorizationServerConfiguration {
	...
	public SecurityFilterChain authorizationServerSecurityFilterChain(HttpSecurity http,
	                                                                  CloudAuthenticationSuccessEventHandler successEventHandler,
	                                                                  CloudAuthenticationFailureEventHandler failureEventHandler) throws Exception {
		// 增加验证码过滤器
		...
		DefaultSecurityFilterChain securityFilterChain = http.authorizeHttpRequests(authorizeRequests -> {
			// 自定义接口、端点暴露
			authorizeRequests.antMatchers("/token/**", "/code/**", "/actuator/**", "/css/**", "/error").permitAll();
