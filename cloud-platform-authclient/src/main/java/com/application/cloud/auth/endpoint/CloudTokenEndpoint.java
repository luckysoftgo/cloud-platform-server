...
import com.application.cloud.admin.api.entity.SysOauthClientDetails;
import com.application.cloud.admin.api.entity.SysTenant;
import com.application.cloud.admin.api.feign.RemoteClientDetailsService;
import com.application.cloud.admin.api.feign.RemoteTenantService;
import com.application.cloud.admin.api.vo.TokenVO;
...
import com.application.cloud.common.security.util.OAuth2ErrorCodesExpand;
import com.application.cloud.common.security.util.OAuthClientException;
...
public class CloudTokenEndpoint {
	...
	public R<Page<TokenVO>> tokenList(@RequestBody Map<String, Object> params) {
		...
		List<TokenVO> tokenVoList = redisTemplate.opsForValue().multiGet(pages).stream().map(obj -> {
			...
			tokenVo.setUserId(cloudUser.getId());
			return tokenVo;
		}).filter(tokenVo -> {
			// 根据用户名过滤
