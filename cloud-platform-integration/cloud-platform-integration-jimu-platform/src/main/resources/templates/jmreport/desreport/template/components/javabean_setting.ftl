            ...
            <div>
                    ...
                    <Row span="24">
                        <i-col span="5">
                            <form-item prop="dbCode" label="编码：">
                                <i-input v-model="formData.dbCode" :disabled="dbCodeDisabled()" style="width: 153px"
                                         type="text" placeholder="请输入编码">
                                </i-input>
                            </form-item>
                        </i-col>
                        <i-col span="5" style="margin-left: 20px;">
                            <form-item prop="dbChName" label="名称：">
                                <i-input v-model="formData.dbChName" type="text" style="width: 200px"
                                         placeholder="请输入名称">
                                </i-input>
                            </form-item>
                        </i-col>
                        <i-col span="3">
                            <form-item prop="isList">
                                <Checkbox v-model="formData.isList" style="width: 100px;margin-left: 10%">是否集合
                                </Checkbox>
                            </form-item>
                        </i-col>
                        <i-col span="3">
                            <form-item prop="isPage" style="display: flex">
                                <Checkbox v-if="formData.isList == true" v-model="formData.isPage"
                                          @on-change="checkChange" style="width: 100px">是否分页
                    ...
                    <Row style="margin-top:10px;">
                        ...
                        <i-col span="10" style="margin-left: 20px;">
                            <form-item prop="javaValue" label="值：">
                                <i-input v-model="formData.javaValue" type="text" style="width: 400px"
                                         placeholder="请填入对应的值"/>
                            </form-item>
                            <i-button @click="handelAnalyze" type="primary">解 析</i-button>
                            <Tooltip :transfer="true" content="javabean解析文档" placement="top">
                                <a class="jimu-table-tip help-color"
                                   href="https://help.jeecg.com/jimureport/dataSet/java.html" target="_blank"
                                   style="font-size: 14px;">
                                    <Icon size="14" type="ios-help-circle-outline" style="margin-top: 2px"/>
                                </a>
                            </Tooltip>
                ...
                <Tabs v-model="tabValue" style="margin-top: 15px">
                    <tab-pane label="报表字段明细" name="1">
                        <i-button type="primary" @click="removeFieldTable" v-if="tab1.selectParamTables.length>0">删除
                        </i-button>
                    ...
                    <tab-pane label="报表参数" name="2">
                        ...
                        <i-button type="primary" @click="removeParamTable" v-if="tab2.selectParamTables.length>0">删除
                        </i-button>
                        <i-table ref="paramTable"
                            ...
                            <template slot="paramTxt" slot-scope="props">
                                <i-form :ref="'formTxtDynamic'+props.idx" :model="props.row">
                                    <form-item prop="paramTxt"
                                               :rules="{required: true, message: '请输入参数文本', trigger: 'change'}"
                                               style="margin-top: 10px">
...
<script>
    ...
    Vue.component('j-javabean-setting', {
        ...
        methods: {
            ...
            setIsPage() {
                $http.get({
                    url: api.queryIsPage(excel_config_id), data: {dbcode: this.formData.dbCode}, success: (result) => {
                        if (result) {
            ...
            checkChange(ispage) {
                if (ispage) {
                    $http.get({
                        ...
                        data: {dbcode: this.formData.dbCode},
                        success: (result) => {
                            if (result) {
                                this.$Modal.confirm({
