    ...
    <div>
        <Submenu :name="'yAxis'+index" style="border-bottom: inset 1px;" v-for="(item,index) in yAxisOptions">
            ...
            <div class="blockDiv" style="padding-bottom: 10px">
                ...
                <Row class="ivurow">
                    <p>Y轴名称&nbsp;&nbsp;</p>
                    <i-input class="rightFontSize iSelect" size="small" v-model="item.name" @on-blur="onYAxisChange"
                             style="margin-left: 4px"></i-input>
                </Row>
                <Row class="ivurow" v-if="isCharHasYMin">
                    <p>最小值&nbsp;&nbsp;</p>
                    <i-input size="small" class="rightFontSize iSelect" style="margin-left: 12px;" v-model="item.min"
                             @on-blur="onYAxisChange"/>
                </Row>
                <Row class="ivurow">
                    <p>分隔线&nbsp;&nbsp;</p>
                    <i-switch size="small" style="margin-left: 128px;" v-model="item.splitLine_show"
                              @on-change="onYAxisChange"/>
                </Row>
                <Row class="ivurow" v-if="typeof item.splitLine_show !== 'undefined' && item.splitLine_show == true">
                    <p>颜色设置&nbsp;&nbsp;</p>
                    <Col>
                    <i-input class="rightFontSize iSelect" v-model="item.splitLine_lineStyle_color" size="small"
                             @on-change="onYAxisChange">
                    <span slot="append">
                        <color-picker class="colorPicker" v-model="item.splitLine_lineStyle_color" :editable="false"
                                      :transfer="true" size="small" @on-change="onYAxisChange"/>
                    </span>
                    </i-input>
                    </Col>
                </Row>
                <Row class="ivurow">
                    ...
                    <i-input class="rightFontSize iSelect" size="small" type="number"
                             v-model="item.axisLabel_textStyle_fontSize" @on-blur="onYAxisChange"></i-input>
                </Row>
                <Row class="ivurow" v-if="typeof item.axisLabel_textStyle_color !== 'undefined'">
                    <p>字体颜色&nbsp;&nbsp;</p>
                    <Col>
                    <i-input class="rightFontSize iSelect" v-model="item.axisLabel_textStyle_color" size="small"
                             @on-change="onYAxisChange">
                    <span slot="append">
                        <color-picker class="colorPicker" v-model="item.axisLabel_textStyle_color" :editable="false"
                                      :transfer="true" size="small" @on-change="onYAxisChange"/>
                    </span>
                    </i-input>
                    </Col>
                ...
                <Row class="ivurow" v-if="typeof item.axisLine_lineStyle_color !== 'undefined'">
                    <p>轴线颜色&nbsp;&nbsp;</p>
                    <Col>
                    <i-input class="rightFontSize iSelect" v-model="item.axisLine_lineStyle_color" size="small"
                             @on-change="onYAxisChange">
                    <span slot="append">
                        <color-picker class="colorPicker" v-model="item.axisLine_lineStyle_color" :editable="false"
                                      :transfer="true" size="small" @on-change="onYAxisChange"/>
                    </span>
                    </i-input>
