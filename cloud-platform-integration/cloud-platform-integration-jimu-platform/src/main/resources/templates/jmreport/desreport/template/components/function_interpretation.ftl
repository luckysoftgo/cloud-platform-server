    ...
    <div>
        ...
        <div v-if="text=='SUM'" class="interpretation">
            ...
            <p>例3：=SUM(A6:C6)，对A6到C6单元格计算最大值
                <Tooltip content="文档" placement="left" class="jimu-tooltip">
                    <span class="jimu-table-tip jimu-icon" @click="questionMarkClick(0)"><Icon size="14"
                                                                                               type="ios-help-circle-outline"/></span>
                </Tooltip>
            </p>
        </div>
        ...
        <div v-if="text=='case'" class="interpretation">
            ...
            <p>最终结果为男，字符串需要加上单引号或者双引号
                <Tooltip content="文档" placement="left" class="jimu-tooltip">
                    <span class="jimu-table-tip jimu-icon" @click="questionMarkClick(1)"><Icon size="14"
                                                                                               type="ios-help-circle-outline"/></span>
                </Tooltip>
            </p>
        </div>
        ...
        <div v-if="text=='date'" class="interpretation">
            ...
            <p>返回日期：1982-08-09 00:00:00
                <Tooltip content="文档" placement="left" class="jimu-tooltip">
                    <span class="jimu-table-tip jimu-icon" @click="questionMarkClick(2)"><Icon size="14"
                                                                                               type="ios-help-circle-outline"/></span>
                </Tooltip>
            </p>
        </div>
        ...
        <div v-if="text=='rand'" class="interpretation">
            ...
            <p>例2：=rand()*100 获得[0, 100)之间的一个随机浮点数
                <Tooltip content="文档" placement="left" class="jimu-tooltip">
                    <span class="jimu-table-tip jimu-icon" @click="questionMarkClick(3)"><Icon size="14"
                                                                                               type="ios-help-circle-outline"/></span>
                </Tooltip>
            </p>
        </div>
        ...
        <div v-if="text=='char'" class="interpretation">
            ...
            <p>例2：=char(101)返回：'e'
                <Tooltip content="文档" placement="left" class="jimu-tooltip">
                    <span class="jimu-table-tip jimu-icon" @click="questionMarkClick(4)"><Icon size="14"
                                                                                               type="ios-help-circle-outline"/></span>
                </Tooltip>
            </p>
        </div>
        ...
        <div v-if="text=='color'" class="interpretation">
            ...
            <p>例1：=color('张三', '#ffffff', '#171516') 给张三设置字体颜色和背景颜色
                <Tooltip content="文档" placement="left" class="jimu-tooltip">
                    <span class="jimu-table-tip jimu-icon" @click="questionMarkClick(5)"><Icon size="14"
                                                                                               type="ios-help-circle-outline"/></span>
                </Tooltip>
            </p>
        </div>
        ...
        <div v-if="text=='assert'" class="interpretation">
            <p>=assert(predicate, [msg])：函数描述：断言函数，当 predicate 的结果为 false 的时候抛出 AssertFailed 异常， msg 错误信息可选。
                <Tooltip content="文档" placement="left" class="jimu-tooltip">
                    <span class="jimu-table-tip jimu-icon" @click="questionMarkClick(6)"><Icon size="14"
                                                                                               type="ios-help-circle-outline"/></span>
                </Tooltip>
            </p>
        </div>
        ...
        <div v-if="text=='seq.array'" class="interpretation">
            <p>=seq.array(clazz, e1, e2,e3, ...)：创建一个指定 clazz 类型的数组，并添加参数 e1,e2,e3 ...到这个数组并返回。 clazz 可以是类似
                java.lang.String 的类型，也可以是原生类型，如 int/long/float 等
                <Tooltip content="文档" placement="left" class="jimu-tooltip">
                    <span class="jimu-table-tip jimu-icon" @click="questionMarkClick(6)"><Icon size="14"
                                                                                               type="ios-help-circle-outline"/></span>
                </Tooltip>
        ...
        <div v-if="text=='seq.contains_key'" class="interpretation">
            <p>=seq.contains_key(map, key)：当 map 中存在 key 的时候（可能为 null），返回 true。对于数组和链表，key 可以是 index，当 index
                在有效范围[0..len-1]，返回 true，否则返回 false
                <Tooltip content="文档" placement="left" class="jimu-tooltip">
                    <span class="jimu-table-tip jimu-icon" @click="questionMarkClick(6)"><Icon size="14"
                                                                                               type="ios-help-circle-outline"/></span>
                </Tooltip>
        ...
        <div v-if="text=='seq.put'" class="interpretation">
            <p>=seq.put(coll, key, value)：用于设置 seq 在 key 位置的值为 value，seq 可以是 map ，数组或者 List。 map 就是键值对， 数组或者 List 的时候，
                key 为索引位置整数，value 即为想要放入该索引位置的值
                <Tooltip content="文档" placement="left" class="jimu-tooltip">
                    <span class="jimu-table-tip jimu-icon" @click="questionMarkClick(6)"><Icon size="14"
                                                                                               type="ios-help-circle-outline"/></span>
                </Tooltip>
        ...
        <div v-if="text=='seq.get'" class="interpretation">
            <p>=seq.get(coll, element)：从 list、数组或者 hash-map 获取对应的元素值，对于 list 和数组， element 为元素的索引位置（从 0 开始），对于 hash map
                来说， element 为 key
                <Tooltip content="文档" placement="left" class="jimu-tooltip">
                    <span class="jimu-table-tip jimu-icon" @click="questionMarkClick(6)"><Icon size="14"
                                                                                               type="ios-help-circle-outline"/></span>
        ...
        <div v-if="text=='reduce'" class="interpretation">
            <p>=reduce(seq,fun,init)：fun 接收两个参数,第一个是集合元素, 第二个是累积的函数,本函数用于将 fun 作用在结果值（初始值为 init
                指定)和集合的每个元素上面，返回新的结果值；函数返回最终的结果值
                <Tooltip content="文档" placement="left" class="jimu-tooltip">
                    <span class="jimu-table-tip jimu-icon" @click="questionMarkClick(6)"><Icon size="14"
                                                                                               type="ios-help-circle-outline"/></span>
                </Tooltip>
        ...
        <div v-if="text=='take_while'" class="interpretation">
            <p>=take_while(seq, pred)：遍历集合 seq，对每个元素调用 pred(x)，返回 true则加入结果集合，最终返回收集的结果集合。也就是说从集合 seq 收集 pred 调用为 true
                的元素
                <Tooltip content="文档" placement="left" class="jimu-tooltip">
                    <span class="jimu-table-tip jimu-icon" @click="questionMarkClick(6)"><Icon size="14"
                                                                                               type="ios-help-circle-outline"/></span>
        ...
        <div v-if="text=='seq.every'" class="interpretation">
            <p>=seq.every(seq, fun)：fun 接收集合的每个元素作为唯一参数，返回 true 或 false。当集合里的每个元素调用 fun 后都返回 true 的时候，整个调用结果为 true，否则为
                false
                <Tooltip content="文档" placement="left" class="jimu-tooltip">
                    <span class="jimu-table-tip jimu-icon" @click="questionMarkClick(6)"><Icon size="14"
                                                                                               type="ios-help-circle-outline"/></span>
        ...
        <div v-if="text=='seq.not_any'" class="interpretation">
            <p>=seq.not_any(seq, fun)：fun 接收集合的每个元素作为唯一参数，返回 true 或 false。当集合里的每个元素调用 fun 后都返回 false 的时候，整个调用结果为
                true，否则为 false
                <Tooltip content="文档" placement="left" class="jimu-tooltip">
                    <span class="jimu-table-tip jimu-icon" @click="questionMarkClick(6)"><Icon size="14"
                                                                                               type="ios-help-circle-outline"/></span>
        ...
        <div v-if="text=='seq.some'" class="interpretation">
            <p>=seq.some(seq, fun)：fun 接收集合的每个元素作为唯一参数，返回 true 或 false。当集合里的只要有一个元素调用 fun 后返回 true 的时候，整个调用结果立即为该元素，否则为
                nil
                <Tooltip content="文档" placement="left" class="jimu-tooltip">
                    <span class="jimu-table-tip jimu-icon" @click="questionMarkClick(6)"><Icon size="14"
                                                                                               type="ios-help-circle-outline"/></span>
        ...
        <div v-if="text=='seq.min'" class="interpretation">
            <p>=seq.min(coll)：返回集合中的最小元素，要求集合元素可比较（实现 Comprable 接口），比较规则遵循 aviator 规则
                <Tooltip content="文档" placement="left" class="jimu-tooltip">
                    <span class="jimu-table-tip jimu-icon" @click="questionMarkClick(7)"><Icon size="14"
                                                                                               type="ios-help-circle-outline"/></span>
                </Tooltip>
            </p>
        </div>
        ...
        <div v-if="text=='DBMAX'" class="interpretation">
            ...
            <p>=DBMAX(db.salary)，对编码为db的数据集中的字段salary进行最大值计算
                <Tooltip content="文档" placement="left" class="jimu-tooltip">
                    <span class="jimu-table-tip jimu-icon" @click="questionMarkClick(8)"><Icon size="14"
                                                                                               type="ios-help-circle-outline"/></span>
                </Tooltip>
            </p>
        </div>
