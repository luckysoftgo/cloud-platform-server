<script type="text/x-template" id="match-setting-template">
    <div class="match_setting rightFontSize">
        <Submenu name="19">
            <template slot="title">
                <span>自定义配色</span>
                <Tooltip placement="top" content="【边缘颜色】选用相对更深色" :transfer="true"
                         v-if="selectedChartType.indexOf('scatter.bubble')!==-1">
        ...
        <Modal :loading="loading" v-model="colorMatchModal" title="自定义配色" :width="30" @on-ok="addColorMatch"
               @on-cancel="onCancel">
                        ...
                        <row>
                            <i-col span="12">
                                <i-input v-model="colorMatch.color" size="small" style="width: 111px;">
                                    <span slot="append">
                                        <color-picker class="colorPicker" v-model="colorMatch.color" :editable="false"
                        ...
                        <row>
                            <i-col span="12">
                                <i-input v-model="colorMatch.edgeColor" size="small" style="width: 111px;">
                                    <span slot="append">
                                        <color-picker class="colorPicker" v-model="colorMatch.edgeColor"
                                                      :editable="false" :transfer="true" size="small"/>
                                    </span>
                                </i-input>
                            </i-col>
                        </row>
                        ...
                        <row>
                            <i-col span="12">
                                <i-input v-model="colorMatch.lineColor" size="small" style="width: 111px;">
                                    <span slot="append">
                                        <color-picker class="colorPicker" v-model="colorMatch.lineColor"
                                                      :editable="false" :transfer="true" size="small"/>
                                    </span>
                                </i-input>
                            </i-col>
                ...
                if (this.chartOptions) {
                    ...
                    arr.forEach(object => {
                        ...
                        if (type == 'bar' || type == 'line') {
                            ...
                            seriesList = object.categories.map(item => {
                                return item.name
                        ...
                        if (i <= this.colorMatchData.length) {
                            ...
                            if (colorMatchDatum) {
                                ...
                                let tempColorStyle = {
                                    type: 'radial',
                                    r: 0.5,
                                    colorStops: [{offset: 0, color: color}, {offset: 1, color: edgeColor}]
                                }
                                if (!item['itemStyle']) {
