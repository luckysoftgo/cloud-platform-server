...
<script type="text/x-template" id="data-source-setting-template">
    <div>
        ...
        >
            <Row>
                <i-col span="4" v-if="sqlForm.dbType == 0">
                    <Icon size="25" v-if="forward=='0'" type="ios-arrow-back"
                          style="float: left;margin-top: -12px;cursor: pointer;" @click="forwardClick('1')"/>
                    <Icon size="25" v-if="forward=='1'" type="ios-arrow-forward"
                          style="float: left;margin-top: -12px;cursor: pointer;border-right: 1px solid #dcdee2;"
                          @click="forwardClick('0')"/>
                </i-col>
            </Row>
            <Row>
                <i-col span="4" v-if="sqlForm.dbType == 0 && forward=='0'">
                    <Card style="height: auto;">
                        <div style="display: flex">
                            <i-select size="small" filterable :model.sync="sqlForm.dbSource" v-model="sqlForm.dbSource"
                                      @on-change="selectdbSource" clearable style="width:170px"
                                      :placeholder="daSourceDesc" @on-clear="handleClear">
                                <i-option v-for="item in maintainData" :value="item.id">{{ item.name }}</i-option>
                            </i-select>
                            <i-button size="small" @click="sourceManage" v-if="sqlForm.dbType == 0" type="primary">维护
                            </i-button>
                            <Tooltip :transfer="true" content="数据源维护文档" placement="top" class="jimu-tooltip">
                                <a class="jimu-table-tip help-color"
                                   href="https://help.jeecg.com/jimureport/base/dbsource.html" target="_blank">
                                    <Icon size="14" type="ios-help-circle-outline" style="margin-top: 2px"/>
                                </a>
                            </Tooltip>
                        </div>
                        <i-input size="small" style="margin-top: 10px" search enter-button v-model="tableName"
                                 placeholder="表名查询" @on-search="queryTableName"/>
                        <div style="margin-top: 10px" v-if="tableList&&tableList.length>0">
                            ...
                            <#--表名列表-->
                            <List size="small" border :loading="tableLoading"
                                  style="overflow-y: auto;height: 720px;margin-top: 10px" ref="taList">
                                <ListItem v-for="(item,index) in tableList" :id="index">
                                    <Tooltip :content="item.name" transfer>
                                        <a href="Javascript:void(0)" @click="tableNameClick(item,index)"
                                           :class="{on:currentIndex === index,'nameCla':index===index}">
                                            {{item.name}}
                                        </a>
                                    </Tooltip>
                                </ListItem>
                ...
                <div v-if="sqlForm.dbType == 0 && forward=='1'"
                     style="height: 880px;box-sizing:border-box;width: 26px;float: left;border-right: 1px solid #dcdee2;">
                </div>
                    ...
                    <div style="margin-left: 20px">
                        <i-form ref="sqlForm"
                            ...
                            <Row span="24">
                                <form-item prop="dbCode" label="编码:">
                                    <i-input :disabled="sqlForm.id!='' && sqlForm.id!=undefined" style="width: 153px"
                                             type="text" v-model="sqlForm.dbCode" placeholder="请输入编码">
                                    </i-input>
                                </form-item>
                                <form-item prop="dbChName" label="名称">
                                    <i-input type="text" style="width: 200px" v-model="sqlForm.dbChName"
                                             placeholder="请输入名称">
                                    </i-input>
                                </form-item>
                                <form-item>
                                    <Checkbox :checked.sync="sqlForm.isList" v-model="sqlForm.isList"
                                              @on-change="isListChange" style="width: 100px">是否集合
                                    </Checkbox>
                                </form-item>
                                <form-item>
                                    <#--<Checkbox :checked.sync="sqlForm.isPage" v-if="addIsPage == true" disabled v-model="sqlForm.isPage">是否分页</Checkbox>-->
                                    <Checkbox :checked.sync="sqlForm.isPage"
                                              v-if="sqlForm.isList == true && sqlForm.dbType != 3"
                                              v-model="sqlForm.isPage" @on-change="checkChange"
                                              style="width: 100px;margin-left: -86px">是否分页
                                        <Tooltip :transfer="true" content="分页文档" placement="top" class="jimu-tooltip">
                                            <a class="jimu-table-tip help-color"
                                               href="https://help.jeecg.com/jimureport/base/paging.html" target="_blank"
                            ...
                            <Row span="24" v-if="sqlForm.dbType == 1">
                                <form-item prop="apiConvert" label="类转换器:">
                                    <i-input style="width:530px" v-model="sqlForm.apiConvert"
                                             placeholder="类转换器"></i-input>
                                </form-item>
                            </Row>
                            <Row style="margin-top: 2%;">
                                <i-col span="24">
                                    <form-item prop="dbDynSql" label="报表SQL:" v-if="sqlForm.dbType == 0">
                                        <i-input v-model="sqlForm.dbDynSql" @on-blur="dbDynSqlBlur" type="textarea"
                                                 :rows="4" placeholder="请输入查询SQL"
                                                 style="min-height: 100px;max-height: 620px;width:950px">
                                        </i-input>
                                        <div style="font-size: 12px;">
                                            <p style="margin-left: 14px">
                                            <ul>
                                                <li>如果id字段为字符串类型则需要加单引号:select * from table where id=${"'$"}${"{id}'"}
                                                    。
                                                </li>
                                                <li>您可以编写${"'$"}${"{id}'"}做为一个参数,这里id是参数的名称。例如:select * from table where
                                                    id=${"'$"}${"{id}'"}。
                                                </li>
                                                <li>您可以编写${'#'}${'{sysUserCode}'} 做为一个系统变量,这里sysUserCode是当前登录人。例如:select
                                                    * from table where create_by=${"'#"}${"{sysUserCode}'"}。
                                                </li>
                                                <li>您可以编写存储过程CALL proc_sys_role(${'$'}${'{'}${'pageNo'}${"}"}
                                                    , ${'$'}${'{'}${'pageSize'}${"}"}),CALL为开启存储过程。
                                                    <Tooltip content="数据源用法文档" placement="top">
                                                        <a class="jimu-table-tip help-color"
                                                           href="https://help.jeecg.com/jimureport/dsUse.html"
                                    ...
                                    <form-item prop="apiUrl" label="Api地址:" v-else-if="sqlForm.dbType == 1">
                                        <i-input v-model="sqlForm.apiUrl" @on-blur="dbApiBlur" type="textarea" :rows="4"
                                        ...
                                        <div style="font-size: 12px;">
                                            ...
                                            <ul>
                                                ...
                                                <li>您可以编写${"#"}${"{sysDateTime}"}
                                                    做为一个系统变量,这里sysDateTime是当前系统时间。例如:http://127.0.0.1:8080/jeecg-boot/jimureport/test?riqi=${"#"}${"{sysDateTime}"}
                                                    。
                                                </li>
                                    ...
                                    <form-item prop="jsonData" label="json数据:" v-else="sqlForm.dbType == 3">
                                        <i-input v-model="sqlForm.jsonData" type="textarea" :rows="4"
                                                 placeholder="请输入JSON数据"
                                                 style="min-height: 120px;max-height: 620px;width:950px">
                                        </i-input>
                                    </form-item>
                                    <i-button @click="handleSQLAnalyze" v-if="sqlForm.dbType == 0" type="primary">
                                        SQL解析
                                    </i-button>
                                    <Tooltip v-if="sqlForm.dbType == 0" :transfer="true" content="SQL解析文档"
                                             placement="top">
                                        <a class="jimu-table-tip help-color"
                                           href="https://help.jeecg.com/jimureport/sql.html" target="_blank"
                                           style="font-size: 14px">
                                            <Icon size="14" type="ios-help-circle-outline" style="margin-top: 2px"/>
                                        </a>
                                    </Tooltip>
                                    <i-button @click="handleApiAnalyze" v-if="sqlForm.dbType == 1" type="primary">
                                        Api解析
                                    </i-button>
                                    <Tooltip v-if="sqlForm.dbType == 1" :transfer="true" content="Api解析文档"
                                             placement="top">
                                        <a class="jimu-table-tip help-color"
                                           href="https://help.jeecg.com/jimureport/api.html" target="_blank"
                                           style="font-size: 14px">
                                            <Icon size="14" type="ios-help-circle-outline" style="margin-top: 2px"/>
                                        </a>
                                    </Tooltip>
                                    <i-button @click="handleJsonAnalyze" v-if="sqlForm.dbType == 3" type="primary">
                                        JSON解析
                                    </i-button>
                                    <Tooltip v-if="sqlForm.dbType == 3" :transfer="true" content="JSON解析文档"
                                             placement="top">
                                        <a class="jimu-table-tip help-color"
                                           href="https://help.jeecg.com/jimureport/dataSet/json.html" target="_blank"
                                           style="font-size: 14px;">
                                            <Icon size="14" type="ios-help-circle-outline" style="margin-top: 2px"/>
                                        </a>
                                    </Tooltip>
                        ...
                        <Tabs v-model="tabValue" style="margin-top: 15px">
                            <tab-pane label="报表字段明细" name="1">
                                <i-button type="primary" @click="removeFieldTable"
                                          v-if="tab1.selectParamTables.length>0">删除
                                </i-button>
                                <i-table style="padding-bottom: 10%;" ref="dynamicTable" @on-select="selectField"
                                         @on-select-all="selectFieldAll" @on-select-all-cancel="cancelFieldAll"
                                         @on-select-cancel="cancelField" stripe :columns="tab1.columns"
                                         :data="tab1.data" :height="tableHeight">
                                    <template slot-scope="{ row, index }" slot="extJsonFiled">
                                        <i-input size="small" v-model="row.extJson"
                                                 @on-change="extJsonFiledChange(index,row.extJson)"
                                                 @on-click="extJsonFiledClick(index,row.extJson)" icon="md-contract"
                                                 class="pointerIcon" placeholder="请输入参数配置"/>
                                    </template>
                                </i-table>
                            </tab-pane>
                            <tab-pane label="报表参数" name="2" v-if="sqlForm.dbType != 3">
                                <i-button type="primary" @click="addParamTable">新增</i-button>
                                <i-button type="primary" @click="removeParamTable"
                                          v-if="tab2.selectParamTables.length>0">删除
                                </i-button>

                                <i-table class="param-style" ref="paramTable" @on-select="selectParam"
                                         @on-select-all="selectParamAll" @on-select-all-cancel="cancelParamAll"
                                         @on-select-cancel="cancelParam" stripe :columns="tab2.columns"
                                         :data="tab2.data" :height="paramTableHeight">
                                    ...
                                    <template slot-scope="{ row, index }" slot="extJsonParam">
                                        <i-input size="small" v-model="row.extJson"
                                                 @on-change="extJsonParamChange(index,row.extJson)"
                                                 @on-click="extJsonParamClick(index,row.extJson)" icon="md-contract"
                                                 class="pointerIcon" placeholder="请输入参数配置"/>
                                    </template>
                                </i-table>
                            ...
                            <tab-pane label="数据预览" name="3" v-if="sqlForm.dbType == 0">
                                ...
                                <i-table ref="viewParamTable" stripe :columns="tab3.columns" :data="tab3.data"
                                         :loading="tab3Loading"></i-table>
        ...
        <Modal
            ...
            <p slot="header">
              <span>数据源维护
                 <Tooltip style="cursor: pointer;float: right;margin-right: 30px;" v-if="sourceDraggable"
                          :transfer="true" content="缩小" placement="top">
                     <Icon type="ios-contract" @click="dataSourceClick"/>
                 </Tooltip>
                 <Tooltip style="cursor: pointer;float: right;margin-right: 30px;" v-else :transfer="true" content="放大"
                          placement="top">
        ...
        <Modal :loading="loading" v-model="visibleData" title="数据源" :width="35" @on-cancel="clearDbSou"
               @on-ok="saveDataSource">
            <div style="padding-right: 30px">
                <i-form ref="dataSource" :model="dataSource" :rules="dataFormValidate" label-colon :label-width="100">
                    ...
                    <form-item prop="dbPassword" label="密码" style="height:50px;width: 100%;">
                        <i-input style="width: calc(100% - 60px)" type="password" password
                                 v-model="dataSource.dbPassword" placeholder="请输入密码"></i-input>
        ...
        <Modal
                ...
                title="确认删除">
            <p>
                <Icon type="ios-alert" color="#f90" size="20px"></Icon>
                是否删除选中数据?
            </p>
        </Modal>
        <Modal
                ...
                title="确认删除">
            <p>
                <Icon type="ios-alert" color="#f90" size="16px"></Icon>
                是否删除选中配置?
            </p>
        </Modal>
        <Modal
        ...
        <Modal draggable :loading="loading" v-model="extJsonShow" title="输入值" :width="1000" @on-ok="extJsonOk"
            ...
            <i-form label-colon :label-width="100">
                <form-item label="参数配置">
                    <i-input :autosize="{minRows: 10}" type="textarea" v-model="paramConfigData"
                             placeholder="请输入参数配置"></i-input>
                </form-item>
            </i-form>
        </Modal>
    ...
    Vue.component('j-data-source-setting', {
        ...
        methods: {
            ...
            editById(dbId) {
                this.tabValue = "1";
                $http.get({
                    url: api.loadDbData(dbId), success: (result) => {
            ...
            setIsPage() {
                $http.get({
                    url: api.queryIsPage(excel_config_id), data: {dbcode: this.sqlForm.dbCode}, success: (result) => {
                        if (result) {
                            this.sqlForm.isPage = false
                        } else {
                            this.sqlForm.isPage = true
            ...
            getReport() {
                $http.get({
                    url: api.getReport(excel_config_id), success: (result) => {
                        // console.log("result====>",result)
                        if (result) {
            ...
            checkChange(ispage) {
                if (ispage) {
                    $http.get({
                        ...
                        data: {dbcode: this.sqlForm.dbCode},
                        success: (result) => {
                            if (result) {
                                this.$Modal.confirm({
            ...
            selectField(selection, row) {
                this.tab1.selectParamTables = [...this.tab1.selectParamTables, {
            ...
            selectParam(selection, row) {
                this.tab2.selectParamTables = [...this.tab2.selectParamTables, {
                    "tableIndex": row.tableIndex,
                    "id": row.id
                }];
            },
            ...
            initDataSource() {
                $http.get({
                    url: api.initDataSource, success: (result) => {
                        let reportResult = result;
                        if (!reportResult) {
                            return;
                        }
            ...
            maintainDataSource() {
                ...
                $http.get({
                    url: api.getDataSourceByPage, data: params, success: (result) => {
