<template>
        ...
        <div style="padding-right: 30px">
            <i-form ref="designerObj" :model="designerObj" :label-width="90" :rules="designerObjRules">
                ...
                <form-item label="类型" prop="type">
                    <i-select :model.sync="designerObj.type" v-model="designerObj.type" style="width:100%"
    ...
    <Modal :loading="loading" v-model="addEchart" title="数据编辑" :width="50" @on-ok="addEchartData">
        <div>
            <i-form>
                <form-item>
                    <i-input type="textarea" :autosize="{minRows: 15,maxRows: 15}"
                             v-model="apiStaticDataList"></i-input>
                </form-item>
            </i-form>
        </div>
    </Modal>
    ...
    <Modal :loading="loading" v-model="chartModule" :width="1000" @on-ok="okAddChart" @on-cancel="selectedChartType=''">
        <p slot="header">
            <span>添加图表
               <Tooltip :transfer="true" content="图表文档" placement="top">
                  <a class="jimu-table-tip help-color" href="https://help.jeecg.com/jimureport/chart.html"
                     target="_blank"><Icon size="14" type="ios-help-circle-outline" style="margin-top: 2px"/></a>
               </Tooltip>
            </span>
        </p>
        <Tabs value="bar" class="chart-modal-content">
            <tab-pane :label="obj.label" :name="obj.name" v-for="(obj,index) of chartTypeList" :index="index+1">
                <Row justify="center">
                    <i-col span="5" offset="1" v-for="(item,index) of obj.typeList"
                           :class="item.allowed ? '':'no-allowed'" style="margin-top: 20px">
                        <div style="border: solid 1px #dcdee2;width: 180px;height: 130px;"
                             :class="selectedChartId == item.id ? 'chart-selected':''" @click="setSelectCharType(item)">
                            <img :src="'${base}'+'${customPrePath}'+item.imgUrl"
                                 style="width:95%;height:95%;margin: 0 5px;">
    ...
    <Modal :loading="loading" v-model="seriesModal" title="系列类型信息" :width="30" @on-ok="addSeriesType"
           @on-cancel="seriesObj={}">
            ...
            <i-form :model="seriesObj" label-colon :label-width="90">
                <form-item label="系列">
                    <i-select v-model="seriesObj.name" style="width:100%" @on-change="selectmenuList" placeholder="请选择">
                        <i-option v-for="(item, index) in customColorNameList" :index="index" :value="item">{{ item }}
                        </i-option>
                    </i-select>
                </form-item>
    ...
    <#--自定义表达式弹窗-->
    <Modal :draggable="funDraggable" :loading="loading" :fullscreen="functionScreen" v-model="customExpressionShow"
           :width="1000" :autosize="true" @on-ok="expressionSave" @on-cancel="expressionCancel" class="expression">
        <p slot="header">
            <span>添加表达式
               <Tooltip :transfer="true" content="表达式文档" placement="top">
                   <a class="jimu-table-tip help-color" href="https://help.jeecg.com/jimureport/function.html"
                      target="_blank"><Icon size="14" type="ios-help-circle-outline" style="margin-top: 2px"/></a>
               </Tooltip>
               <Tooltip style="cursor: pointer;float: right;margin-right: 30px;" v-if="functionScreen" :transfer="true"
                        content="缩小" placement="top">
        ...
        <i-form label-colon :label-width="90">
            ...
            <Row justify="center" style="margin-top: 10px">
                <i-col>
                    <i-input type="textarea" v-model="expression" placeholder="请输入表达式"
                             :class="functionScreen==false?'expressionInput':'expressionHeight'"></i-input>
                </i-col>
            </Row>
            <Row justify="center" style="margin-top: 10px">
                <i-col span="6" class="functionDiv">
                    <div :class="leftFunctionIndex == item.id?'leftFunctionSelect':'leftFunction'"
                         v-for="(item,index) in functionType" @click="leftFunctionClick(item.id)">
                ...
                <i-col span="13">
                    <div class="childrenDiv">
                        <div v-if="commonFunction" v-for="(item,index) in newFunctionList"
                             @click="rightFunctionClick(item,index)"
                             :class="rightFunctionIndex == index?'rightFunctionSelect':'activeItem'">
    ...
    <#-- 增强弹框 -->
    <Modal draggable :loading="loading" v-model="enhanceModalVisible" title="增强配置" :width="1000"
           :class="'jmreport-enhance'"
