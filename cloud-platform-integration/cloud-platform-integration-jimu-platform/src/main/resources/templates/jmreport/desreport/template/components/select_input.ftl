...
<script type="text/x-template" id="select-input-template">
    <div class="jimu-cust-component">
        <i-select v-if="selectFlag" filterable size="small" :value="selectedValue" :transfer="true" :clearable="true"
                  @on-change="selectChange">
            <i-option v-for="(item, index) in selectOptions" :value="item.value" :key="index">{{ item.title }}
            </i-option>
        </i-select>
