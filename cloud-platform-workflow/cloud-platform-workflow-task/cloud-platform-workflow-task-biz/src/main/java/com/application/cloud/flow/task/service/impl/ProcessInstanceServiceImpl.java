...
import com.application.cloud.admin.api.entity.SysUser;
...
import com.application.cloud.flow.task.api.feign.RemoteFlowEngineService;
import com.application.cloud.flow.task.constant.FormTypeEnum;
import com.application.cloud.flow.task.constant.NodeStatusEnum;
import com.application.cloud.flow.task.constant.NodeUserTypeEnum;
import com.application.cloud.flow.task.constant.ProcessInstanceConstant;
import com.application.cloud.flow.task.dto.*;
import com.application.cloud.flow.task.entity.Process;
import com.application.cloud.flow.task.entity.*;
import com.application.cloud.flow.task.service.*;
import com.application.cloud.flow.task.utils.NodeFormatUtil;
import com.application.cloud.flow.task.vo.FormItemVO;
import com.application.cloud.flow.task.vo.NodeFormatParamVo;
import com.application.cloud.flow.task.vo.NodeVo;
import com.application.cloud.flow.task.vo.ProcessCopyVo;
...
public class ProcessInstanceServiceImpl implements IProcessInstanceService {
	...
	public R formatStartNodeShow(NodeFormatParamVo nodeFormatParamVo) {
		...
		if (StrUtil.isNotBlank(nodeFormatParamVo.getTaskId())) {
			...
			if (!r.isOk()) {
				...
				Map<String, Object> variableMap = objectMapper.readValue(data, new TypeReference<Map<String, Object>>() {
				});
	...
	public R detail(String processInstanceId) {
		...
		List<FormItemVO> jsonObjectList = objectMapper.readValue(oaForms.getFormItems(), new TypeReference<List<FormItemVO>>() {
		});
